...
Slider::Slider(float h, float w, float posx, float posy, bool f, bool sit, float mul)	: situation(sit) , fill(f) , multiple(mul)
{
	height = h;
	width = w;
...
void Slider::update(sf::RenderWindow& window , float scroll)
{
	...
	{
		smallmultiple = (float)(1 / multipleS);
	}
...
//description
void Slider::displayDescription(const char * desc, sf::Font & f, unsigned int h)
{
	font = f;
	fontSize = h;
...
void Slider::displayValue(double min, double max,int prec,bool s)
{
	minValue = min;
	maxValue = max;
	precision = prec;
